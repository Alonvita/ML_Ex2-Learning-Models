...
def loss_and_gradients(x, y, params):
    ...
    for i in range(len(params) // 2 - 2, -1, -1):
        W = params[i * 2]
...
def train_epoch(train_data, dev_data, learning_rate, params):
    ...
    for x, y in train_data:
        loss, grads = loss_and_gradients(x, y - 1, params)
        cum_loss += loss
...
def load_data(mnist_pkl):
    ...
    train_set, valid_set, test_set = cloudpickle.load(f)
    f.close()
...
def check_gradients():
 ...
 #   from grad_check import gradient_check
    ...
    for _ in range(10):
        ...
        for p in my_params:
            print("error")#gradient_check(_loss_and_p_grad, p)


def main():
  #  TRAIN, DEV, TEST = load_data('data/mnist.pkl.gz')
    TRAIN = read_data('data/train.csv')
    DEV = read_data('data/validate.csv')
    TEST = read_data('data/test.csv')

    params = create_classifier([3072, 512, 100, 10])
    num_iterations = 40
    ...
    for I in range(num_iterations):
        ...
        print(str(I) + " , " + str(loss) + " , " + str(train_accuracy) + " , " + str(dev_accuracy) + " , " + str(elapsed))
